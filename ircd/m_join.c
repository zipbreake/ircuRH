 ...
 *
 * $Id: m_join.c,v 1.2 2006/12/26 10:44:00 zipbreake Exp $
 */
...
CMD_FUNC(m_join)
{
  ...
  for (name = ircd_strtok(&p, chanlist, ","); name;
	...
	/* Canales protegidos por tabla j */
	...
	if (chptr) {
		if ((chptr->rhmode.mode & RHMODE_ONLYIRCOP) && !IsAnOper(sptr)) {
			send_reply(sptr, ERR_CANTJOIN, chptr->chname, "Solo ircops pueden ingresar en el canal (+O)");
			continue;
		}
