 ...
 *
 * $Id: m_mode.c,v 1.1.1.1 2006/12/19 12:54:37 zipbreake Exp $
 */
...
m_mode(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (!(member = find_member_link(chptr, sptr)) ||
	  !(member->status & (CHFL_CHANOP | CHFL_OWNER | CHFL_HALFOP))) {
    if (IsLocalChannel(chptr->chname) && HasPriv(sptr, PRIV_MODE_LCHAN)) {
      ...
      return modebuf_flush(&mbuf);
		 ...
		 if (es_representante(sptr) || IsOper(sptr) || IsPreoper(sptr)) {
			 modebuf_init(&mbuf, sptr, cptr, chptr,
  ...
  if (IsChanHalfop(member) && !IsChanOp(member) && !IsChanOwner(member))
...
ms_mode(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (IsServer(sptr)) {
    ...
    if ((!(member = find_member_link(chptr, sptr)) ||
		!(member->status & (CHFL_CHANOP | CHFL_OWNER | CHFL_HALFOP))) &&
		(!es_representante(sptr) && !IsPreoper(sptr) && !IsOper(sptr) && !IsBot(sptr))) {
      modebuf_init(&mbuf, sptr, cptr, chptr,
      ...
      mode_parse(&mbuf, cptr, sptr, chptr, parc - 2, parv + 2,
		...
		if (!IsBot(sptr) && !es_representante(sptr) && !IsPreoper(sptr) && 
			member && IsChanHalfop(member) && !IsChanOp(member) && !IsChanOwner(member))
		{
